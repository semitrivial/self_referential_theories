...
%
%We avoid this obstacle by using the
%
...
%
%$\alpha=\max(\onset(\phi^+_0))+1$ (we define $\max(\emptyset)=0$).
%
\item If $\phi$ is $\neg\psi$, $\psi\rightarrow\rho$, or $\forall x\psi$, then
$\phi^+$ is $\neg\psi^+$, $\psi^+\rightarrow\rho^+$ or $\forall x\psi^+$, respectively.
%
%\item If $\phi$ is $\neg\psi$ then $\phi^+$ is $\neg\psi^+$.
%\item If $\phi$ is $\psi\rightarrow\rho$ then $\phi^+$ is $\psi^+\rightarrow\rho^+$.
%\item If $\phi$ is $\forall x\psi$ then $\phi^+$ is $\forall x\psi^+$.
%
\end{enumerate}
...
%
%\begin{lemma}
%\label{plusandscommute}
%If $i\in\N$, $s$ is an assignment,
%and $\bullet^+$ is an $i$-stratifier,
%then \[\bullet^s\circ \bullet^+=\bullet^+\circ\bullet^s.\]
%That is, for every $\LPA(\omega)$-formula $\phi$,
%$(\phi^+)^s=(\phi^s)^+$.
%\end{lemma}
%
%\begin{proof}
...
%\obj(10,10)[bb]{$\LPA(\indset)$}
%\obj(-10,0)[cc]{$\LPA(\omega)/\FV$}
%\obj(10,0)[dd]{$\LPA(\indset)/\FV$}
%\mor{aa}{bb}{$+$}
%\mor{aa}{cc}{$s$}[\atright,\solidarrow]
%\mor{bb}{dd}{$s$}
%\mor{cc}{dd}{$+$}%[\atright,\solidarrow]
%\enddc\]
%
%\end{multicols}
%
