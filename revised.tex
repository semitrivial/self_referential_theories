...
$j\succ i$. No such modification is needed in the following lemma. This is
...
\begin{enumerate}
    ...
    \item $[\mbox{$j$-SMT}]_i$.
    \item (If $i\not\prec j$) $\T\cup [S]_i$, for any stratifiable-r.e.-generic
    $\T=(T_k)_{k\in\omega}$,
    where $S$ is the schema: $\Pr j\phi$ ($\phi\in T_j$).
\end{enumerate}
\end{corollary}
...
\begin{enumerate}
    ...
    that $\T^2\cup [S_2]_i$ is stratifiable-r.e.-generic if $i\not\prec j$.
\end{enumerate}
