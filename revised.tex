...
\begin{itemize}
\item
First, we will aim the following preliminary result.
...
$\M^{I(i)}\models U_i\cup T_i$.
    \begin{itemize}
    \item
    Part of proving $\M^{I(i)}\models U_i$ will be proving
    $\M^{I(i)}\models\ucl{\Prr\alpha i\phi\rightarrow\phi}$
    whenever this is $i$-stratified.
    ...
    \item
    Part of proving $\M^{I(i)}\models U_i$ will be proving
    $\M^{I(i)}\models\ucl{\Prr j\phi\rightarrow\phi^+}$
    ...
    \item
    Once we establish $\M^{I(i)}\models U_i$, we will essentially
    be able to conclude
    $\M^{I(i)}\models T_i$ using Theorem \ref{stratificationtheorem}.
...
\[
\M_\T\models \Pr i [s] \mbox{ iff } T_i\models \phi^s.
\]
