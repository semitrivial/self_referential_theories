...
\begin{definition}
Suppose $i\in\omega$.
A set $T$ of $\LPA(\indset)$-sentences is \emph{$i$-uniform}
if the following conditions hold:
\begin{enumerate}
\item $T$ is $i$-stratified.
\item Whenever $\phi\in T$,
...
Suppose $T$ is an $i$-uniform $\LPA(\indset)$-theory.
\begin{enumerate}
...
\begin{proof}
Note that since $T$ is $i$-uniform, in particular $T$ is $i$-stratified.
...
\end{itemize}
An $\LPA(\indset)$-theory $T$ is \emph{$i$-stratifiable} if
...
\end{enumerate}
A family $\T=(T_i)_{i\in\omega}$ is \emph{stratifiable} if each $T_i$ is
$i$-stratifiable.
\end{definition}

The reason for the word \emph{stratifiable} in Definition
...
Suppose $T$ is an $i$-stratifiable $\LPA(\indset)$-theory.
Then:
\begin{enumerate}
...
\begin{proof}
Note that since $T$ is $i$-stratifiable, in particular $T$
is $i$-uniform and hence, $i$-stratified.
...
\begin{enumerate}
  ...
  $\mathscr N^+ \models \Pr i\phi[s]$ if and only if $\mathscr N\models (\Pr i\phi)^+[s]$.
  \item If $\phi$ is not an $\LPA(\omega)$-formula, then
  $\mathscr N^+ \models \Pr i\phi[s]$ if and only if $\mathscr N\models \Pr i\phi[s]$.
\end{enumerate}
\end{definition}

Case 2 in Definition \ref{moduloidefn} is somewhat arbitrary.
We will only ever really care about whether
$\mathscr N^+ \models \Pr i\phi[s]$ when $\Pr i\phi$ is $j$-stratified for some $j$.
...
The next definition needs motivation.  We seek
a true stratified family $\T=(T_i)_{i\in\omega}$
where $T_i$ declares the truth of $T_j$ ($j\preceq i$).
By collapsing $\T$ we hope to fulfil the second promise from the
introduction.  For $j\prec i$, $T_i$ cannot contain
$\ucl{\Pr j\phi\rightarrow\phi}$: it is not $i$-stratified (e.g.~if
$\phi$ is $\Pr i(1=0)$).  An alternative is
$\ucl{\Pr j\phi\rightarrow\phi^+}$ where $\bullet^+$ ranges over
$i$-stratifiers.  But imagine proving
$\M_\T\models \ucl{\Pr j\phi\rightarrow\phi^+}$.  We would
assume $T_j\models\phi^s$ and try to show $\M_\T\models\phi^+[s]$.
Since $j\prec i$ we could assume $\M_\T\models T_j$, so
$\M_\T\models \phi^s$, so $\M_\T\models\phi[s]$---not what we want.
If only $\M^+_\T\models\phi[s]$, Lemma \ref{structurecollapsingmagic}
would give $\M_\T\models\phi^+[s]$. In a sense, $\bullet^+$ is invisible
to $T_j$ ($T_j$ can't see $T_i$ because $j\prec i$).  So our proof
that $\M_\T\models T_j$ could've gone through for $\M^+_\T$.
The following definition will allow the proof to go through
for $\M^+_\T$ via strong induction hypothesis.

\begin{definition}
\label{equividefn}
For every $i\in\omega$,
let $\bullet\myequiv_i\bullet$ be the smallest transitive binary relation on $\LPA(\indset)$-structures
satisfying the following:
\begin{enumerate}
  \item For any $\LPA(\indset)$-structure $\M$, $\M\myequiv_i\M$.
  \item For any $\LPA(\indset)$-structure $\M$, any $i\prec j$, and any computable $j$-stratifier $\bullet^+$,
    $\M\myequiv_i \M^+$.
\end{enumerate}
\end{definition}
...
If $\M$ interprets formulas by substitution, then
$\M'$ interprets formulas by substitution.
\end{lemma}

\begin{proof}
We may assume $\M'=\M^+$ for some computable $j$-stratifier $\bullet^+$,
$j\succ i$.
We are given that for every $\LPA(\indset)$-formula $\phi$ and assignment $s$,
$\M\models\phi[s]$ if and only if $\M\models\phi^s$.
We must show that for every such $\phi$ and $s$, $\M'\models\phi[s]$ if and only
if $\M'\models\phi^s$.

We induct on $\phi$.
By Definition \ref{equividefn}, $\M^+$ and $\M$ agree on all symbols
except possibly $\Pr j$, and they agree on $\Pr j \phi_0$ when
$\phi_0$ is not an $\LPA(\omega)$-formula.
Thus the only nontrivial case is when $\phi$
is of the form $\Pr j\phi_0$ for some $\LPA(\omega)$-formula $\phi_0$.
...
Suppose the $\LPA(\indset)$-structure $\M$ is an instance of
Definition \ref{intendedstructdefn}.
For any $i\in\omega$,
for any $\LPA(\indset)$-structure $\M'$ such that $\M\myequiv_i\M'$,
$\M$ interprets formulas by substitution.
\end{lemma}
...
is an r.e.~family of $i$-uniform $\LPA(\indset)$-theories.
We say $\T$ is \emph{stratified-r.e.-generic} if
for every stratifiable r.e.~family $\U\supseteq\T$, every $i\in\omega$,
and every $\LPA(\indset)$-structure $\M$,
if $\M_{\str\U}\myequiv_i\M$, then $\M\models T_i$.
\end{definition}
...
\[
  \mbox{
    ...
    j(\psi\wedge(\phi\vee\neg\phi))}$ whenever this is $j$-stratified.
  }
\]
...
is r.e.
In each
case below, let $\U=(U_k)_{k\in\omega}$ be a stratifiable r.e.~family extending the family
...
\item
(2) Assume $\M_{\hat\U}\myequiv_i \M$, we must show
...
\item
(6)
By (4), $\M\models\mbox{$i$-Assigned Strativalidity}$
whenever $\M_{\hat\U}\myequiv_i\M$.  Assume $\M_{\hat U}\myequiv_j\M$,
...
By Lemma \ref{sequencelemma}, $j\prec i$ and there is an $i$-stratifier
...
Since $\M$ and $\M_{\hat\U}$ agree on $\Pr i$,
$\M\models\Pr i\phi[s]$.
...
Use the fact that the stratifiers in Definition \ref{equividefn}
are computable.
...
every $\alpha\in\epom$ and every $\LPA(\indset)$-structure $\M$,
if $\M_\S\myequiv_i\M$ then $\M\models S_j\cup S_{(\alpha,j)}$.
...
every $\alpha\in\epom$,
and every $\LPA(\indset)$-structure $\M$,
if $\M_\S\myequiv_j\M$, then $\M\models S_k\cup S_{(\alpha,k)}$.
\end{quote}
Fix $\M$ and assume $\M_\S\myequiv_i\M$.
For all $j\prec i$,
Lemma \ref{sequencelemma} says $\M_\S\myequiv_j \M$,
...
T_j &\models \phi^s
  &\mbox{(Definition of $\M_\S$)}\\
\M^+ &\models \phi^s
  &\mbox{(By Lemma \ref{sequencelemma}, $\M_\S\myequiv_j\M^+$, so by ($*$), $\M^+\models T_j$)}\\
\M &\models (\phi^s)^+
  &\mbox{(Lemma \ref{structurecollapsingmagic})}\\
\M &\models (\phi^+)^s
  &\mbox{(Clearly $(\phi^s)^+\equiv(\phi^+)^s$)}\\
