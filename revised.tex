...
\begin{itemize}
\item
We will consider a carefully-chosen
family $\U=(U_i)_{i\in\omega}$ of $\LPA(\indset)$-theories such that each
$U^-_i=T_i$. We will argue that the truth of $\str{\U}$ implies the truth of $\T$,
and then we will show the truth of $\str{\U}$.
\item
To deal with the difficulty mentioned at the beginning of Section \ref{stratifiersection},
we will prove more than necessary, to obtain a strong induction hypothesis.
Namely, we will prove, by $\prec$-induction, that $\forall i\in\omega$,
for every stratifier-set $I(i)$ above $i$,
$\M_{\str{\U}}^{I(i)}\models U_i\cup T_i$.
    \begin{itemize}
    \item
    Part of proving $\M_{\str{\U}}^{I(i)}\models U_i$ will be proving
    $\M_{\str{\U}}^{I(i)}\models\ucl{\Prr\alpha i\phi\rightarrow\phi}$
    whenever this is $i$-stratified.
    This, at last, is where we will use induction on $\alpha$ (enabled by our careful
    development of stratification in Section \ref{stratificationsection}).
    \item
    Part of proving $\M_{\str{\U}}^{I(i)}\models U_i$ will be proving
    $\M_{\str{\U}}^{I(i)}\models\ucl{\Prr j\phi\rightarrow\phi^+}$
    whenever $j\prec i$, $\phi$ is an $\LPA(\omega)$-formula,
    and $\bullet^+$ is an $i$-stratifier. This is where we will take advantage
    of our strong induction hypothesis.
    \item
    Once we establish $\M_{\str{\U}}^{I(i)}\models U_i$, we will essentially
    be able to conclude
    $\M_{\str{\U}}^{I(i)}\models T_i$ using Theorem \ref{stratificationtheorem}.
    To do so, we will induct on formula complexity.
    \end{itemize}
\end{itemize}
