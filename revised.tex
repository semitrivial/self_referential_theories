...
Instead, we will induct on ordinals (namely, ordinals up to $\epom$).
There are no ordinal numbers in sight so far, and in order to make ordinal
...
we will focus on theories $T^\alpha_i$ (the intended interpretation
of $\Prr \alpha i \phi$ will be that
$T^\alpha_i\models\phi$).
Each $T^\alpha_i$ will contain copies of axioms of $T_i$, except that these
axiom-copies will have exponents $<\alpha$ inserted strategically.
Conversely, every axiom in $T_i$ will have such a copy in $T^\alpha_i$ for
some sufficiently large $\alpha$.
For large enough ordinals $\alpha$, we will show that these theories $T^\alpha_i$
\emph{collapse} in a technical sense: loosely speaking, beyond a certain point,
...
By proving (using induction on $\alpha$) that the $T^\alpha_i$
hold, we will be able to conclude that the corresponding $T_i$ also hold.
All the exact details of this process will be spelled out below, but for now we
will motivate informally.

Suppose we desire $T_i$ to contain the axiom $\Pr i(1+1=2)$.
Then, as we carry out the procedure in the above paragraph, we will ensure that
...
Since $1+1=2$ is non-controversial, it makes sense to include it from
the very beginning in the increasing family of theories $T^\alpha_i$.

Next, suppose that for every arithmetical sentence $\phi$,
we would like $T_i$ to include
$\Pr i\phi\rightarrow \Pr i\Pr i\phi$. Then we will arrange that for
all $\alpha_0<\alpha_1<\alpha$, $T^\alpha_i$ will contain
$\Prr {\alpha_0} i\phi \rightarrow \Prr {\alpha_1} i \Prr {\alpha_0} i \phi$.
The intended interpretation of $\Prr {\alpha_0} i \phi$ shall be
$T^{\alpha_0}_i\models \phi$. Thus, it would make no sense for us to put
the axiom
$\Prr {\alpha_0} i\phi \rightarrow \Prr {\alpha_0} i \Prr {\alpha_0} i \phi$
into $T^\alpha_i$: the fact that $T^{\alpha_0}_i\models \phi$
does not necessarily imply that $T^{\alpha_0}_i\models \Prr \alpha_0 i \phi$,
especially since $T^{\alpha_0}_i$ is limited to formulas in which all exponents
are $<\alpha_0$. At least
$\Prr {\alpha_1} i\phi \rightarrow \Prr {\alpha_0} i \Prr {\alpha_0} i \phi$
is plausible (it could be satisfied by $T^{\alpha_1}_i$ containing some basic
axioms about $\Prr {\alpha_0} i$, axioms which $T^{\alpha_0}_i$ cannot itself
contain if $T^{\alpha_0}_i$ is restricted to exponents $<\alpha_0$).

Again, suppose that for some $j\prec i$, we would like for $T_i$ to include
$\Pr i(\Pr j (1=0)\rightarrow (1=0)) \rightarrow \Pr i(\Pr j(1=0)) \rightarrow \Pr i(1=0)$.
We will arrange
that for each $\alpha$, the family $T^\alpha_i$ will contain (for each $\alpha_0<\alpha$):
$\Prr {\alpha_0} i(\Pr j (1=0)\rightarrow (1=0))
\rightarrow \Prr {\alpha_0} i(\Pr j (1=0)) \rightarrow \Prr {\alpha_0} i (1=0)$.
Note the lack of exponents on $\Pr j$.
The intuition is that $\cup_\alpha T^\alpha_i$ should be thought of as a kind of
``awakened'' version of $T_i$ which has somehow become more self-aware and realized
something about its own structure (namely, that said structure arises from an
increasing family of theories), but it has only gained such insight about itself,
not about any other agents. (There is a good technical reason for this. We would like
...
true in general, since the number $n$ is the same in both of them.)

Similarly, suppose we would like $T_i$ to include
$\Pr j(\Pr i (1=0))\rightarrow \Pr i(1=0)$.
We will arrange that for each $\alpha$, the family $T^\alpha_i$ will contain
(for each $\alpha_0<\alpha$): $\Pr j(\Pr i(1=0))\rightarrow \Prr \alpha i(1=0)$.
Note the lack of exponent on the $\Pr i$ within the scope of $\Pr j$.
The intuition is just as in the previous paragraph. We think of $\cup_\alpha T^\alpha i$
as an awakened version of $T_i$ which has gained further insight into the structure of
$T_i$. But the structure of $T_i$ is quite divorced from that of $T_j$, even
from what $T_j$ says about $T_i$.
