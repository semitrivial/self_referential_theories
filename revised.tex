...
$\mathscr L$-structure with standard first-order part, then $\mathscr M$
is said to \emph{interpret formulas by substitution} if the following
...
For any $i\in\omega$,
any $\LPA(\indset)$-structure $\M'$ such that $\M\myequiv_i\M'$,
any $\LPA(\indset)$-formula $\phi$ and any assignment $s$,
$\M'\models\phi[s]$ if and only if $\M'\models\phi^s$.
\end{lemma}
