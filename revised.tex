...
But there are no ordinals anywhere in sight. In order to obtain ordinals
to induct on, we will modify the theories we care about, in a process we
call \emph{stratification}. The technical details are somewhat complicated,
so we will start with some informal motivational remarks. Readers who
would like to advance directly to the formal definitions can safely
skip to Subsection \ref{StratificationDetailsSection}.
...
\]
We will arrange
that for each $\alpha$, $T^\alpha_i$ will contain (for each $\alpha_0<\alpha$):
\[
...
\]
We will arrange that for each $\alpha$, $T^\alpha_i$ will contain
...
The intuition is as in the previous paragraph. We think of $\cup_\alpha T^\alpha_i$
as a version of $T_i$ with extra information about the structure of
$T_i$. It does not have any such extra information about the
...
\begin{enumerate}
    \item
    For any $j\in\omega$ and $\alpha\in\epom$, if $\Prr\alpha j$ occurs in $\phi$,
    then $j=i$.
    \item
