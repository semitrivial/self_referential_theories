...
By definition of $\M_{\T'}$, $\M_{\T'}\models \Pr j\phi[s]$.  By arbitrariness
of $s$, $\M_{\T'}\models \Pr j\phi$.
\end{proof}
...
is $i$-uniform and hance, $i$-stratified.
...
\mbox{$\mathscr N^+ \models \Pr i\phi[s]$
 if and only if $\mathscr N\models (\Pr i\phi)^+[s]$.}
\]
\end{definition}
...
Use the fact that the stratifiers in Definiton \ref{equividefn}
are computable.
