...
It remains to show $\M_{\U}$ satisfies $j$-Validity.
...
Then $\M_{\U}$ satisfies Assigned Validity and $j$-Validity by
...
We now have enough technical machinery to fulfil the second promise from the
...
$(\Pr j\psi)^+\equiv \Prr\beta j\psi^+$, where $\alpha\not=\beta$.
...
to provide enough building blocks so as to be flexible enough to suffice for
creation of background provability theories at least as strong as the axioms
of knowledge and arithmetic chosen in Carlson's paper \cite{carlson2000}.
Corollary \ref{stratifiableregenericsummary} is
...
descriptive set theoretical complexity of $\mathrm{WF}$ is similar to a
