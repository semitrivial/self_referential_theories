...
The following theorem fulfills the first promise from the introduction.
...
of straticlosed-r.e.-genericness, what things
$T_i$ says about $T_j$ need not merely be true,
...
into $[\mbox{$j$-Deduction}]_i$ ($j\preceq i$) is because
...
\item $[\mbox{$j$-Deduction}]_i$ (if $j\prec i$).
...
Since $I(i)$ is above $i$ and $j\prec i$, $\M^{I(i)}$ and $\M$
agree on $\Pr j$, so
...
\item $\T\cup [S]_i$ where
$S$ is the schema $\Pr j\phi^-$ ($\phi\in T_j$, $j\prec i$).
\end{enumerate}
\end{lemma}
...
Let $I(i)$ be any computable stratifier-set above $i$.
Suppose $\phi\in T_j$ where $j\prec i$. Let $s$ be any assignment.
...
\begin{enumerate}
    \item $[\mbox{$j$-Deduction}]_i$ (if $j\preceq i$).
...
\begin{enumerate}
    ...
    that $\T^2\cup [S_2]_i$ is stratifiable-r.e.-generic if $j\prec i$.
\end{enumerate}
