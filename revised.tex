...
and $\bullet^+$ is a computable $i$-stratifier.
...
If $\M$ is an $\LPA(\indset)$-structure and $I$ is a
stratifier-set. If $\M$ interprets formulas by substitution,
then $\M^I$ interprets formulas by substitution.
\end{lemma}

\begin{proof}
By induction on $|I|$, using Lemma \ref{decomposingMtotheI},
it suffices to show that if $\M$ interprets formulas by substitution and
$\bullet^+$ is a computable $i$-stratifier, then $\M^+$ interprets
formulas by substitution.
...
We induct on $\phi$.
By Definition \ref{moduloidefn}, $\M^+$ and $\M$ agree on all symbols
